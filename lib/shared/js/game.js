...
export default class Game {
    constructor() {
        ...
        this.matrix.populateMatrixWithViruses(5);
        // local queue of pills
        this.pillQueue = [];
        // position in the room's pill queue
        this.pillQueuePos = 0;
        this.currentPill = null;
        //possible states: controlling, pill-placed, settling, clearing, top-out, victory, frozen, cheating
    ...
    update() {
        ...
        // this intermediate state is used so that once a pill is placed on the client side, it doesn't get added to the matrix until it reaches the server
        // the main goal of this is to prevent cheating
        // TODO: implement cheating state
        if (this.state == 'pill-placed') {
            this.matrix.addPillToCells(this.currentPill);
            this.addGarbage();
        }
    ...
    getViewMatrix() {
        let m = this.matrix.copy();
        m.addPillToCells(this.currentPill);
        return m;
    }
    //deprecated
    getViewObject() {
