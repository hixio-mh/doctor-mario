...
const PILL_COLORS = ["#ff8a97", "#ffff87", "#7aa2ff"];
const VIRUS_COLORS = ["#eb6e6e", "#f5e56e", "#5988d4"];
            ...
            for (let c = 0; c < matrix.cells[r].length; c++) {
                ...
                if (cell != null) {
                    ...
                    context.fillStyle = cell.isVirus
                        ? VIRUS_COLORS[cell.color]
                        : PILL_COLORS[cell.color];
                    context.fill();

                    context.beginPath();
                    context.lineWidth = 2;
                    context.strokeStyle = theme.cellBorder;
                    if (!cell.isVirus) { // HACK: don't draw border if cell is a pill
