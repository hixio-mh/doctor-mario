...
let storedKeyBinds = localStorage.getItem("keybinds");
if (storedKeyBinds) {
    KEY_BINDS = JSON.parse(storedKeyBinds);
}
...
function onKeyDown(event) {
    ...
    if (
        inputState.CAN_INPUT &&
        !event.repeat &&
        KEY_BINDS[event.keyCode] != null
    ) {
        inputState[KEY_BINDS[event.keyCode]] = true;
        if (
            KEY_BINDS[event.keyCode] == "LEFT" ||
            KEY_BINDS[event.keyCode] == "RIGHT"
        ) {
            if (timeoutKey != null) {
